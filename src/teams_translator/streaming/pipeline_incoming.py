...
from teams_translator.streaming.pipeline_runtime import build_guard, build_vad, reset_asr_utterance, speech_evidence
        ...
        if vad_result.active:
            ...
            if event is not None and event.text.strip():
                ...
                if guard.accepted:
                    ...
                    if decision.should_commit and not decision.remaining_partial_text:
                        self._handle_commit(decision.committed_text, event.audio_start_ns, captured_at_ns, event.model_info)
